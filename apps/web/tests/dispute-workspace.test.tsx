...
import { DisputeDetail, DisputePanel } from "@/features/disputes/view/dispute-workspace";
...
const dispute: Dispute = {
  ...
  explanation: "Successful seller delivery was not recorded; a refund is recommended.",
  createdAt: "2026-09-18T10:00:00Z",
};

describe("dispute workspace", () => {
  it("creates a dispute and shows its deterministic resolution", async () => {
    const createDispute: DisputeAction = vi.fn().mockResolvedValue({ ok: true, value: dispute });
    render(
      <DisputePanel
    ...
    fireEvent.change(screen.getByLabelText("Dispute reason"), { target: { value: "not_delivered" } });
    fireEvent.change(screen.getByLabelText("Statement"), { target: { value: dispute.statement } });
    ...
    expect(screen.getByRole("link", { name: "View dispute record" })).toHaveAttribute(
      "href",
      `/dashboard/disputes/${dispute.disputeId}`,
    );
  });

  it("shows evidence verification and a resolved dispute record", () => {
    render(<DisputeDetail dispute={{ ...dispute, status: "resolved" }} evidenceValid />);
