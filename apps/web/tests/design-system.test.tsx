...
describe("AgentPay web foundation", () => {
  it("renders accessible primary navigation and account actions", () => {
    ...
    expect(within(primaryNavigation).getByRole("link", { name: "Product" })).toHaveAttribute(
      "href",
      "/#product",
    );
    expect(screen.getAllByRole("link", { name: "Sign in" })[0]).toHaveAttribute("href", "/sign-in");
    expect(screen.getAllByRole("link", { name: "Start selling" })[0]).toHaveAttribute(
      "href",
      "/sign-up",
    );
  });
  ...
  it("renders product, company, and legal footer navigation", () => {
    ...
    expect(screen.getByRole("navigation", { name: "Product links" })).toBeVisible();
    expect(screen.getByRole("navigation", { name: "Company links" })).toBeVisible();
    expect(screen.getByRole("navigation", { name: "Legal links" })).toBeVisible();
  });

  it("defines fluid layout tokens and a reduced-motion mode", () => {
    const stylesheet = readFileSync(path.resolve(process.cwd(), "app/globals.css"), "utf8");
