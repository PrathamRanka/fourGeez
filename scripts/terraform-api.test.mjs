...
test("AWS-005 excludes the deferred approval WebSocket runtime", () => {
  ...
  assert.match(architecture, /historical API Gateway approval WebSocket design is not deployed/);
});
...
test("AWS-005 uses the canonical production frontend origin", () => {
  ...
  assert.match(development, /web_origin\s*=\s*"https:\/\/agentpay\.prathamranka\.in"/);
  assert.match(runbook, /https:\/\/agentpay\.prathamranka\.in/);
  assert.doesNotMatch(runbook, /vercel\.app/);
});

test("AWS-005 infrastructure is reproducible but disabled until runtime readiness passes", () => {
  ...
  assert.doesNotMatch(application, /aws_apigatewayv2_api[^]*protocol_type\s*=\s*"WEBSOCKET"/);
