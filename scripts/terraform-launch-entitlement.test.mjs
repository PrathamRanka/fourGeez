...
test("manual launch entitlement role is optional and seller-partition scoped", () => {
  ...
  assert.match(variables, /\$\{var\.aws_account_id\}/);
  ...
  assert.match(operations, /SELLER#sel_\*/);
  assert.match(operations, /sts:RoleSessionName/);
  assert.doesNotMatch(operations, /dynamodb:Scan/);
  assert.doesNotMatch(operations, /dynamodb:DeleteItem/);
  assert.doesNotMatch(operations, /dynamodb:PutItem/);
  assert.match(outputs, /output\s+"project_name"/);
});
