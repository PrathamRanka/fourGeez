...
| ADR-028 | Use the proprietary `POST /v1/integration-access-tokens` bootstrap exchange only through the AgentPay connector for project-key installations; direct remote MCP clients use OAuth protected-resource discovery and never receive a project key. | The project key remains an AgentPay-specific installation credential without making the remote MCP resource incompatible with standards-capable clients. |
...
| ADR-033 | Deliver approval invitation tokens in URL fragments, exchange them once into a server-side multi-session browser grant, and require its Secure HttpOnly cookie plus double-submit CSRF and exact Origin for decisions. | URL fragments avoid referrer leakage, concurrent approval sessions do not overwrite one another, and cookie-authenticated mutations receive explicit CSRF protection. |
