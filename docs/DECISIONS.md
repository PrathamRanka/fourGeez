...
| ADR-054 | Keep `purchaseIntent` and `transaction` as the canonical commerce lifecycle instead of introducing an `Order` aggregate. A buyer may cancel only an unclaimed `ready` intent; checkout conditionally claims that intent as `executed` before issuing the first payment challenge, and the exclusive `expiresAt` boundary wins any race while the intent is still `ready`. An intent claimed before that boundary remains `executed`, and its deterministic transaction may complete or recover after the intent window closes. Public intent and transaction responses include a deterministic single-product exact-price breakdown, while transaction responses also include a derived external-commerce lifecycle and recovery projection. Seller-recorded external refunds remain append-only assertions and never become network proof or automatic fund movement. | This gives external consumers order-compatible status, totals, cancellation, and remediation visibility while preserving immutable commercial terms, preventing cancellation/payment races, and keeping AgentPay non-custodial. |
| ADR-055 | Publish a public runtime payment-capability catalog and negotiate only across capabilities enabled by the configured adapter. The testnet runtime exposes exact x402 on Base Sepolia USDC only. Browser wallets use the same intent and `/pay` path as agents. Recovery distinguishes same-request retry, same-proof retry after durable verification, fresh authorization, and new checkout; it never silently adds card, mainnet, another network or asset, custody, or automatic refunds. | Buyers and agents need deterministic compatibility and recovery without risking a second authorization when settlement outcome is unknown. |
