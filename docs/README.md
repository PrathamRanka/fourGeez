...
OpenAPI 0.10 adds stateless external-buyer x402 compatibility detection with
explicit buyer-agent authentication and protocol-header requirements. It does
not issue credentials, accept seller project keys, or operate a buyer agent.
It retains OpenAPI 0.9 authoritative pre-payment input-schema validation, the
schema-driven human buyer form, and deterministic paid-but-unfulfilled recovery
with one bounded
...
Local Playwright verification now purchases the same seeded product once
through browser checkout and once through the external-agent simulation, then
checks that both unique transactions appear once in the seller dashboard and
are labeled by buyer channel in the separate Test activity view. This is local mock
evidence only; it does not satisfy REL-003 or REL-008 deployed testnet proof.
...
| [runbooks/SECURITY_REGRESSION.md](runbooks/SECURITY_REGRESSION.md)                               | Deterministic issue 61-64 gate and sanitized deployed evidence sequence                           |
