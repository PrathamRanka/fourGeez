...
5. The buyer requests the paid route with the intent identifier.
6. The gateway returns an x402 challenge when payment is absent.
7. The buyer wallet authorizes the exact asset, network, amount, destination,
   and resource, then retries with payment proof.
8. The gateway rechecks current entitlement, verifies and settles payment, and
   records finality.
9. One conditional-write winner claims the finalized transaction for
...
11. The proxy forwards exactly once with the execution capability and returns
