...
M7.1 acceptance: one clean command starts the production-shaped local system and one Ctrl+C removes it; a seller can register, subscribe, resume onboarding, verify a wallet and domain, connect a coding agent, publish readable products, receive required notifications, and operate the dashboard against the real Go backend; buyers and agents can discover active products through the central directory and complete the same x402 commerce pipeline; cancellation or key revocation prevents new network participation even when seller code continues running or is forked; stale discovery cannot authorize a transaction; already-finalized payments fulfill exactly once under the documented obligation rule; operators can investigate and safely suspend/recover the system; buyer remediation, data lifecycle, support, API compatibility, cost controls, backups, restore drills, and product analytics are operational; every user-facing surface passes the final frontend polish review; all security, accessibility, responsive, performance, integration, contract, and E2E tests pass; and `scripts/flaws.md` contains no untracked launch requirement.
