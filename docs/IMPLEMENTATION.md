...
- [!] **AWS-005** Deploy the Go Lambda behind one HTTP API serving REST and the `/mcp` endpoint, with stages, throttles, and access logs. The historical approval WebSocket remains disabled and is not deployed for Lean V1. The durable ARM64 runtime and additive plan were verified on September 19, 2026; direct Lambda health and dependency-readiness checks passed for DynamoDB, S3, KMS, Secrets Manager, and the x402 facilitator. Runtime payment configuration is locked to credential-free x402 testnet (`https://x402.org/facilitator`, Base Sepolia `eip155:84532`, USDC `0x036CbD53842c5426634e7929541eC2318f3dCF7e`). The Mumbai Lambda concurrency quota now reports 400, resolving the former quota-10 blocker, but `api_deployment_enabled` remains false and no Lambda or HTTP API is deployed until a fresh reviewed plan is approved.
- [x] **AWS-006** Configure Cognito seller authentication. The Mumbai
      ...
      JavaScript. API Gateway JWT protection is prewired for seller-only route
      families and remains disabled with AWS-005.
- [ ] **AWS-007** Configure Bedrock model access and runtime permissions.
- [-] **AWS-008** Deploy Next.js and configure environment-specific API origins. The Vercel project and required production variable names exist, and a dry-run-first script now validates and installs the non-secret Terraform outputs plus an optional in-memory session key. Final origin-value verification remains blocked until AWS-005 produces `http_api_url`.
- [-] **AWS-009** Add CloudWatch dashboards and alarms for API, MCP, checkout, facilitator, evidence, seller forwarding, webhook retry/dead-letter, Lambda errors/throttles, API latency, and 5xx responses. Terraform and stable structured log signals are implemented with missing-data-safe low-cost settings; deployment and notification delivery remain unverified until AWS-005 is applied.
