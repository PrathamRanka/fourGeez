...
## Milestone M7.3 — Public discovery extension
...
M7.3 acceptance: people and software agents can start from one public capability
manifest, list or exactly filter currently eligible published products without
a production table scan, follow canonical storefront/product links, and see
truthful supported-channel and x402 testnet capability metadata. Directory and
manifest data remain non-authoritative for purchases.
...
- [!] **AWS-005** Deploy the Go Lambda behind one HTTP API serving REST and the `/mcp` endpoint, with stages, throttles, and access logs. The historical approval WebSocket remains disabled and is not deployed for Lean V1. The durable ARM64 runtime and additive `13 add, 0 change, 0 destroy` plan were verified on September 19, 2026; direct Lambda health and dependency-readiness checks passed for DynamoDB, S3, KMS, Secrets Manager, and the x402 facilitator. Runtime payment configuration is locked to credential-free x402 testnet (`https://x402.org/facilitator`, Base Sepolia `eip155:84532`, USDC `0x036CbD53842c5426634e7929541eC2318f3dCF7e`). Activation is blocked because this new Mumbai account has a regional Lambda concurrency quota of 10 and AWS requires all 10 executions to remain unreserved, so the required non-zero function reservation cannot be applied. Quota request `694ba3ebe3a0453c817f0968791d0e4dVjy5XZm6` is open with AWS. All partially created application resources were rolled back; foundation, Cognito, protected evidence, secrets, and budget resources remain deployed.
