...
- [x] **WEB-021** Refine the public stack proof around seller outcomes, remove non-brand surface tinting, and coordinate a readiness-driven homepage preloader with the hero headline reveal. Preserve server-rendered headline semantics, derive progress from document/font/critical-asset readiness, avoid a fixed fake loading delay, and bypass prolonged motion when reduced motion is requested. Depends on WEB-020.
...
- [x] **AWS-005** Deploy the Go Lambda behind one HTTP API serving REST and the `/mcp` endpoint, with stages, throttles, and access logs. The Mumbai development runtime is deployed on ARM64 with durable DynamoDB repositories, protected S3 evidence objects, KMS evidence/capability signing, Secrets Manager peppers and seller secrets, the Lambda HTTP adapter, Cognito JWT routes, a public default route for protocol-specific authentication, seven-day structured logs, and API Gateway rate/burst throttles. The account's regional concurrency quota is 10, so development uses unreserved Lambda concurrency while retaining the API throttle; demo and production must restore a reviewed positive reservation after quota approval. The historical approval WebSocket remains disabled and is not deployed for Lean V1.
- [x] **AWS-006** Configure Cognito seller authentication. The Mumbai
      ...
      JavaScript. API Gateway JWT protection is active for seller-only route
      families in the deployed AWS-005 HTTP API.
