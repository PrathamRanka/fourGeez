...
## Milestone M7.1 — Launch hardening, subscription enforcement, and full integration

This milestone is required before AWS deployment or release verification. It
turns the completed M7 feature surfaces into one production-shaped product and
closes the launch gaps recorded in `scripts/flaws.md`.

The threat model, cloud-versus-seller responsibility matrix, cancellation
policy, discovery/authorization split, Redis keys, audit requirements,
TypeScript/Node.js reference folder structure and code, and required security
tests in `scripts/flaws.md` are implementation inputs for this milestone. The
Go modular monolith remains authoritative for identity, subscription,
authorization, payment, transaction, evidence, and publication rules. The Node
implementation is limited to the optional local MCP connector, seller request
verification package, and contract-equivalent reference adapters; it must not
become a second control plane.

- [ ] **LCH-001** Reconcile `README.md`, `PRODUCT.md`, `ARCHITECTURE.md`, `DATA_MODEL.md`, `SECURITY.md`, `TEST_PLAN.md`, `DECISIONS.md`, the API contracts, and this ledger so that launch status, authentication, subscription enforcement, buyer channels, storefront routes, and current limitations agree. Depends on WEB-010.
- [ ] **LCH-002** Lock the public information architecture: the public site explains AgentPay, the authenticated dashboard serves sellers, `/store/{sellerSlug}` represents one seller, `/store/{sellerSlug}/products/{productSlug}` represents one product, human checkout lives on the product page, and the current buyer page becomes the clearly labeled `/demo/agent-checkout` agent-channel demonstration. Depends on LCH-001.
- [ ] **LCH-003** Extend the catalog contract and data model with a seller-approved human product display name and readable public product slug while preserving `routeId`, `pathPattern`, method, MIME type, and upstream details as explicit technical fields. Define uniqueness, normalization, immutable references, migration, SEO metadata, manifest, and historical receipt behavior. Depends on LCH-001.
- [ ] **LCH-004** Update OpenAPI, MCP, AsyncAPI when affected, seller verification, discovery, receipt, and webhook contracts for short-lived capabilities, entitlement revisions, inactive discovery responses, execution authorization, revocation, and the complete documented error taxonomy. Depends on LCH-001–LCH-003.
- [ ] **LCH-005** Define the seller subscription and entitlement lifecycle independently from buyer settlement: `active`, bounded `grace`, `suspended`, `cancelled` with exact `accessEndsAt`, and `closed`; document provider-event mapping, cancellation-at-period-end, non-payment suspension, reactivation, credential rotation, fraud quarantine, and historical read-only access. Verify and record the production subscription-billing provider before implementing provider-specific callbacks. Depends on LCH-001.
- [ ] **LCH-006** Define the non-bypassable seller-MCP security boundary: seller-hosted code is untrusted, the official MCP remains cloud-authoritative, the seller API key is only a bootstrap credential, discovery never authorizes transactions, and no seller-side fork receives signing, payment, publication, or transaction authority. Depends on LCH-004–LCH-005.
- [ ] **LCH-007** Replace the JavaScript read-only demo API with one production-shaped local launcher for Next.js, the real Go API, the demo seller, the mock x402 facilitator, approval WebSocket support, deterministic buyer fallback, and in-memory repositories. The launcher must validate configuration, wait for readiness, print useful URLs, surface failed dependencies, and terminate every process after one Ctrl+C on Windows. Depends on LCH-004.
- [ ] **LCH-008** Implement a named, development-only Go seed profile containing a launch-ready seller, an incomplete seller, below-threshold and approval-required products, payment-pending/fulfilled/failed/disputed transactions, valid and invalid evidence, webhook success/retry/dead-letter attempts, and asset/network-separated analytics. Seed/reset code must be impossible to enable in production. Depends on LCH-007.
- [ ] **LCH-009** Add process liveness and dependency readiness checks for configured persistence, signing, payment/facilitator, seller forwarding, WebSocket, and cache dependencies. Health must not report readiness when a transaction-critical dependency is unavailable. Depends on LCH-007.
- [ ] **LCH-010** Implement the authoritative seller entitlement projection with status, exact UTC `accessEndsAt`, monotonically increasing `entitlementEpoch`, source revision, plan/features, and optimistic concurrency. Subscription provider events must be authenticated, idempotent, ordered, replay-safe, and persisted before cache invalidation. Depends on LCH-005.
- [ ] **LCH-011** Harden project API keys with at least 256 bits of randomness, a separate public credential identifier, one-time raw display, keyed digest storage using a cloud-held pepper, constant-time comparison, scopes, expiry, last-used metadata, rotation, revocation, and rate-limited exchange. Raw keys must never be persisted or logged. Depends on LCH-006 and LCH-010.
- [ ] **LCH-012** Add a standards-aligned token exchange that accepts the project API key only as a bootstrap/client credential and returns a 2–5 minute seller-scoped access token. The token must include issuer, audience, subject, credential ID, seller ID, scopes, entitlement epoch, JTI, issued time, and expiry, with `Cache-Control: no-store`. Depends on LCH-011.
- [ ] **LCH-013** Implement asymmetric short-lived capability signing and verification through a cloud-only KMS/HSM or managed authorization boundary, publish rotated public verification keys through JWKS, pin accepted algorithms and audiences, and test overlapping key rotation. Private signing material must never reach seller infrastructure, repositories, browser code, or ordinary environment files. Depends on LCH-012.
- [ ] **LCH-014** Replace permanent integration credentials on ordinary MCP requests with short-lived access-token authorization. Middleware must verify signature, issuer, audience, expiry, JTI shape, seller, credential, exact tool scope, current entitlement epoch, active subscription, quota, ownership, idempotency, and explicit seller confirmation for commercial mutations. Depends on LCH-010–LCH-013.
...
- [ ] **LCH-031** Replace internal-first terminology throughout the UI: storefront name, public storefront URL, service API URL, product name, product URL, decimal price, verified payment destination, project connection key, test purchase, publish product, and payment/delivery proof. Put route ID, path pattern, HTTP method, MIME type, network internals, timeout, and atomic amounts under an advanced technical section. Depends on LCH-002–LCH-003.
...
- [ ] **LCH-038** Revamp public documentation and frontend quality: lead with a clear API-to-paid-product thesis, present the setup as a visual commerce rail, include copyable clean-checkout/install/configure/start/seed/verify/stop/reset/troubleshooting instructions, distinguish local mock/testnet/production behavior, use existing Aceternity/21st.dev/shadcn components before custom UI, use maintained shadcn/21st.dev charts, preserve Server Components, remove data waterfalls, support keyboard/reduced-motion use, and verify 360/768/1280/1440 layouts. Depends on LCH-031–LCH-037.
...
M7.1 acceptance: one clean command starts the production-shaped local system and one Ctrl+C removes it; a seller can register, resume onboarding, verify a wallet, connect a coding agent, publish readable products, and operate the dashboard against the real Go backend; browser and agent buyers complete the same x402 commerce pipeline; cancellation or key revocation prevents new network participation even when seller code continues running or is forked; stale discovery cannot authorize a transaction; already-finalized payments fulfill exactly once under the documented obligation rule; all security, accessibility, responsive, integration, contract, and E2E tests pass; and `scripts/flaws.md` contains no untracked launch requirement.
