...
- [x] **WEB-025** Make the product dashboard the seller control plane for every mutable product-contract field, keep verified payment configuration and immutable product identity read-only, reject live-contract edits, and require the explicit draft -> validate -> preview -> approve-and-publish sequence with optimistic version and contract-hash binding. Verified locally with focused Go/web tests, web lint/typecheck/build, contract lint, and diff checks. Depends on WEB-024 and EXT-005.

- [x] **WEB-026** Resolve buyer-checkout issues 25-29 by rendering ordinary browser input from the signed published `inputSchema`, labeling required fields accurately, returning exact inline field errors, and enforcing the bounded schema again in the Go checkout domain before transaction creation, x402 challenge issuance, verification, or settlement. Validation covers required and unknown properties, JSON types, enums/constants, nested objects and arrays, string/item/number bounds, patterns, formats, and supported schema compositions. The agent demonstration retains its explicit JSON editor but uses the same schema validation. Verified locally with focused frontend and Go regression tests plus the affected lint, typecheck, build, and diff checks; deployed verification was not performed. Depends on EXT-005 and LCH-013-LCH-014.

- [x] **WEB-027** Resolve buyer-checkout issues 30–34: show full buyer and seller wallet addresses before authorization without relying on wallet account labels, expose the real transaction/request trace, remove corrupted typography, and discard stale intent/challenge/signature state when recovery requires a new checkout. Preserve exact-price semantics, sanitized errors, accessibility, reduced motion, and the pure-black design system. Verified locally with focused checkout and payment tests plus affected lint, typecheck, build, and diff checks; deployed verification was not performed. Depends on WEB-026, LCH-013–LCH-014, and EXT-004.

- [x] **WEB-028** Strengthen global AgentPay brand discovery with self-canonical English and `x-default` alternates, truthful worldwide service-area structured data, continent-representative social locales, visible worldwide discovery copy, and an expanded root `llms.txt` that identifies authoritative machine-readable entry points and current testnet limits. Do not create duplicate regional doorway pages or claim guaranteed ranking. Verified locally with the complete web test suite, repository lint, web typecheck, and production web build. The full repository typecheck remains blocked by the pre-existing missing `@aws-sdk/lib-dynamodb` installation in `verification/node`; deployed indexing verification was not performed. Depends on WEB-016 and SEO-003.
...
- [x] **AWS-005** Deploy the Go Lambda behind one HTTP API serving REST and the `/mcp` endpoint, with stages, throttles, and access logs. The historical approval WebSocket remains disabled and is not deployed for Lean V1. On September 19, 2026, Terraform applied 37 additions, 0 changes, and 0 destroys, and the post-apply plan reported no changes. On September 20, 2026, a refresh-only plan identified the emergency Lambda and IAM patches; the reviewed reconciliation applied 0 additions, 3 in-place changes, and 0 destroys, deployed the current artifact, tightened the launch-entitlement operator role, and ended with a no-change plan. The same day, CloudWatch attribution of a 15-second timeout led to a reviewed 0-add, 3-change, 0-destroy deployment that set the Lambda and HTTP integration to 29 seconds, bounded seller forwarding at 25 seconds with a two-second parent-deadline reserve, and added path plus integration/response latency and integration-error access-log fields. A later reviewed 0-add, 1-change, 0-destroy apply changed only the Lambda's public API and public-base origins to the branded Vercel BFF, followed by a no-change plan. Mumbai Lambda `agentpay-dev-api` is Active on `provided.al2023` ARM64 with reserved concurrency 5 under the regional quota of 400. HTTP API `sadmp7j94e` remains the private server-only upstream at `https://sadmp7j94e.execute-api.ap-south-1.amazonaws.com`; public links use `https://agentpay.prathamranka.in/api/backend`. The branded `/health/live`, `/.well-known/agentpay`, and `/v1/discovery/products` routes return `200` without exposing the raw origin. Runtime payment configuration remains locked to credential-free x402 testnet (`https://x402.org/facilitator`, Base Sepolia `eip155:84532`, USDC `0x036CbD53842c5426634e7929541eC2318f3dCF7e`).
...
- [x] **AWS-008** Deploy Next.js and configure environment-specific API origins. Vercel keeps the Terraform HTTP API origin server-only and exposes the branded same-origin `/api/backend/*` BFF. On September 20, 2026, Git commit `eb9bb19` deployed successfully through the Vercel Git integration; `/api/backend/health/live` and `/api/backend/.well-known/agentpay` returned `200`.
...
- [x] **AWS-012** Deploy the durable route/entitlement publication outbox processor with revision recovery, bounded retry, partial-batch failure handling, dead-letter retention, and completion observability. On September 20, 2026, Terraform enabled the `agentpay-dev-main` `NEW_IMAGE` stream, attached a filtered Lambda event source with three retries and batch bisection, and created the managed-encryption `agentpay-dev-publication-outbox-dlq`. A direct production-runtime invocation returned an empty failure list and a consistent DynamoDB read verified its `publicationCompletion` record. The current V1 runtime has no distributed cache: publication invalidation is intentionally a no-op before authoritative snapshot refresh, and credential authorization reads DynamoDB directly. Redis/CDN remain a separate optional scaling task under AWS-011.
- [-] **AWS-013** Configure and verify authenticated subscription-provider callbacks, event replay protection, environment separation, cancellation scheduling, entitlement projection, and secret rotation independently from buyer x402 settlement. While Stripe is disabled, cloud environments require an explicit dry-run-first, environment- and version-bound, assumed-role-only launch-entitlement operation. The reviewed plan digest and exact confirmation bind the seller and complete operation; an immutable seller-scoped operation claim makes exact replay idempotent and conflicting replay fail closed; the claim, reconciliation, projection, and administrator audit record commit atomically. No seller API or normal UI can grant entitlement. Webhook signature/history canary tooling is implemented. Stripe callbacks and seller-facing webhook replacement/disable remain pending.
