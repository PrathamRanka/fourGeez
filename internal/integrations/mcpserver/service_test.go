...
func TestServiceReadsSellerScopedResources(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			mimeType: JSONMIMEType,
			contains: []string{"agentpay.setup.v1", ".mcp.json", "AGENTPAY_PROJECT_KEY", "local-mcp-connector"},
		...
		},
		{
			...
			mimeType: JSONMIMEType,
			contains: []string{"agentpay.setup.v1", ".codex/config.toml", "AGENTPAY_PROJECT_KEY", "local-mcp-connector"},
			excludes: []string{"integration_test_token"},
		},
		{
			...
			mimeType: JSONMIMEType,
			contains: []string{"agentpay.setup.v1", "stdio", "AGENTPAY_PROJECT_KEY", "local-mcp-connector"},
			excludes: []string{"integration_test_token"},
		},
		{
			name:     "Claude Code setup bundle v2",
			uri:      ClaudeCodeSetupV2ResourceURI,
			mimeType: JSONMIMEType,
			contains: []string{`"schemaVersion":"agentpay.setup.v2"`},
		},
		{
			name:     "Codex setup bundle v2",
			uri:      CodexSetupV2ResourceURI,
			mimeType: JSONMIMEType,
			contains: []string{`"schemaVersion":"agentpay.setup.v2"`},
		},
		{
			name:     "generic MCP setup bundle v2",
