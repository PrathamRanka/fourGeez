...
17. **Resolved and deployed (2026-09-20):** paid-route and entitlement versions atomically write immutable DynamoDB publication outbox events. The Mumbai table stream invokes the Lambda consumer with partial-batch failure reporting, three bounded retries, batch bisection, and a managed-encryption SQS dead-letter queue. A live invocation refreshed the seller publication and wrote the idempotent completion record. Redis/CDN remain separately deferred under issue 69; credentials continue to use authoritative DynamoDB reads, so no credential-cache invalidation is required in the current V1 profile.
...
63. [x] Seller integration rotation already revokes the predecessor atomically and securely replays the reveal-once successor response. Webhook rotation now supports replacement overlap followed by authenticated, version-guarded, idempotent predecessor disablement; disabled subscriptions receive no new deliveries. Deployed webhook receiver and retry-drain evidence remains pending while Stripe and external webhook certification stay disabled.
...
68. [x] The durable route/entitlement publication outbox is deployed through the `agentpay-dev-main` DynamoDB `NEW_IMAGE` stream. The Lambda consumer filters immutable outbox inserts, reports individual batch failures, retries three times with batch bisection, sends exhausted records to the encrypted `agentpay-dev-publication-outbox-dlq`, and conditionally stores completion. A live invocation returned no failures and its completion record was verified with a consistent DynamoDB read. Redis/CDN remain excluded under issue 69, and current credential checks read authoritative DynamoDB state directly.
...
71. [x] The applied Mumbai Lambda regional concurrency quota is 400. The API keeps reserved concurrency at 5, leaving the required unreserved capacity and a strict low-cost burst guard; Terraform validates the quota before deployment.
...
73. [x] **Resolved and deployed (2026-09-20):** Public API traffic now uses the stable Vercel BFF origin at `https://agentpay.prathamranka.in/api/backend`. The generated API Gateway origin remains a server-only Vercel upstream, browser cookies are not forwarded, x402 headers and response streams are preserved, and recursive proxy configuration fails closed. Terraform applied 0 additions, 1 in-place Lambda environment change, and 0 destroys; the post-apply plan reported no changes. The branded health and platform-manifest routes return `200`, and public discovery no longer emits an `execute-api` URL. Nginx and an API Gateway custom domain were intentionally avoided to keep the Lambda architecture low-cost.
